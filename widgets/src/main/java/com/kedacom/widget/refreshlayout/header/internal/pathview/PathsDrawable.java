...
import com.kedacom.widget.refreshlayout.internal.PaintDrawable;
...
//    }
    ...
    public void setBounds(int left, int top, int right, int bottom) {
        ...
        if (mltOriginPath != null && mltOriginPath.size() > 0 && (width != mWidth
            || height != mHeight)) {
            int ox = mStartX, oy = mStartY;
            float ratioWidth = 1f * width / mOriginWidth;
            float ratioHeight = 1f * height / mOriginHeight;
            mPaths = PathParser
                .transformScale(ratioWidth, ratioHeight, mltOriginPath, mltOriginSvg);
            if (!onMeasure()) {
                mWidth = width;
                mHeight = height;
...
//    }
    ...
    private void createCachedBitmapIfNeeded(int width, int height) {
        if (mCachedBitmap == null || width != mCachedBitmap.getWidth() || height != mCachedBitmap
            .getHeight()) {

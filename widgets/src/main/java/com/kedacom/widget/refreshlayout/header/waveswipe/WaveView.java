...
package com.kedacom.widget.refreshlayout.header.waveswipe;
...
//  protected Paint mShadowPaint;
    ...
    /**
     * ベジェ曲線を引く際の座標 左側の2つのアンカーポイントでいい感じに右側にも
     */
...
//        setUpPath();
    ...
    /**
     * Viewのサイズが決まったら {@link WaveView#mWidth} に横幅 {@inheritDoc}
     */
    @Override
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        final View thisView = this;
        updateMaxDropHeight(
            (int) Math.min(Math.min(w, h), thisView.getHeight() - mDropCircleRadius));
        super.onSizeChanged(w, h, oldw, oldh);
    }

    /**
     * 描画されてから {@link WaveView#mMaxDropHeight} を更新する {@inheritDoc}
     */
    @Override
...
//        mDropRect.setEmpty();
        ...
        double c =
            Math.pow(q - circleCenterY, 2) + Math.pow(circleCenterX, 2) - Math
                .pow(mDropCircleRadius,
                    2);
...
//    }
    ...
    public void appearPhase(float move1, float move2) {
        ...
        mWavePath.cubicTo(
            ...
            mWidth * Math.max(BEGIN_PHASE_POINTS[1][1] + move1 - move2, APPEAR_PHASE_POINTS[1][1]),
            mWidth - mWidth * APPEAR_PHASE_POINTS[0][0], mWidth * APPEAR_PHASE_POINTS[0][1], mWidth,
            0);
    ...
    public void expandPhase(float move1, float move2, float move3) {
        ...
        mWavePath.cubicTo(mWidth * EXPAND_PHASE_POINTS[0][0], mWidth * EXPAND_PHASE_POINTS[0][1],
            mWidth * Math.min(
                ...
                Math.max(BEGIN_PHASE_POINTS[1][1] + move1 - move2, APPEAR_PHASE_POINTS[1][1])
                    - move3,
                EXPAND_PHASE_POINTS[1][1]),
            mWidth * Math.max(BEGIN_PHASE_POINTS[2][0] - move2, EXPAND_PHASE_POINTS[2][0]),
            mWidth * Math.min(
                Math.max(BEGIN_PHASE_POINTS[2][1] + move1 - move2, APPEAR_PHASE_POINTS[2][1])
                    + move3,
        ...
        mWavePath.cubicTo(mWidth * Math.min(
            ...
            mWidth * Math.min(
                ...
                Math.min(BEGIN_PHASE_POINTS[0][1] + move1 + move2, APPEAR_PHASE_POINTS[5][1])
                    + move3,
        ...
        mWavePath.cubicTo(
            ...
            mWidth * Math.min(
                ...
                Math.max(BEGIN_PHASE_POINTS[3][0] - move2, APPEAR_PHASE_POINTS[3][0]) + move3,
                EXPAND_PHASE_POINTS[3][0]), mWidth * Math.min(
                Math.min(BEGIN_PHASE_POINTS[3][1] + move1 + move2, APPEAR_PHASE_POINTS[3][1])
                    + move3,
                EXPAND_PHASE_POINTS[3][1]),

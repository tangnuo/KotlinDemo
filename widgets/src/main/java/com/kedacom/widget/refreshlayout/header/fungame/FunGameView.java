package com.kedacom.widget.refreshlayout.header.fungame;
...
public abstract class FunGameView<T extends FunGameView> extends FunGameBase {
    ...
    public FunGameView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        int maskTextSizeTop = (int) TypedValue
            .applyDimension(TypedValue.COMPLEX_UNIT_SP, 16, metrics);
        int maskTextSizeBottom = maskTextSizeTop * 14 / 16;

        maskTextSizeTop = ta
            .getDimensionPixelSize(R.styleable.FunGameView_fghMaskTextSizeTop, maskTextSizeTop);
        maskTextSizeBottom = ta.getDimensionPixelSize(R.styleable.FunGameView_fghMaskTextSizeBottom,
            maskTextSizeBottom);
        ...
        if (!thisView.isInEditMode()) {
            int height = SystemUtil.dp2px(100);
...
//            maskLp.bottomMargin = (int) FunGameView.DIVIDING_LINE_SIZE;
        ...
        DIVIDING_LINE_SIZE = Math.max(1, SystemUtil.dp2px(0.5f));
    ...
    private void drawText(Canvas canvas, int width, int height) {
        switch (status) {
            ...
            case STATUS_GAME_PLAY:
                mPaintText.setTextSize(SystemUtil.dp2px(25));
                promptText(canvas, mTextLoading, width, height);
                break;
            case STATUS_GAME_FINISHED:
                mPaintText.setTextSize(SystemUtil.dp2px(20));
...
//                mMaskViewTop.setTitle(com.scwang.smartrefresh.layout.R.string.srl_header_secondary);
//                break;
        }
    }
    ...
    public void onStartAnimator(@NonNull RefreshLayout layout, int height, int maxDragHeight) {
        ...
        animatorSet.play(ObjectAnimator
            .ofFloat(topView, "translationY", topView.getTranslationY(), -mHalfHeaderHeight))
            .with(ObjectAnimator.ofFloat(bottomView, "translationY", bottomView.getTranslationY(),
                mHalfHeaderHeight))

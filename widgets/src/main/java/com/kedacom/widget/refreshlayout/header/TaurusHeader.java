package com.kedacom.widget.refreshlayout.header;

import static com.kedacom.util.SystemUtil.dp2px;
...
import com.kedacom.widget.refreshlayout.header.internal.pathview.PathsDrawable;
import com.kedacom.widget.refreshlayout.internal.InternalAbstract;
import java.util.HashMap;
import java.util.Map;
import java.util.Random;

/**
 * Taurus from https://github.com/Yalantis/Taurus
 */
...
//    private boolean mEndOfRefreshing;
    ...
    public TaurusHeader(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        thisView.setMinimumHeight(dp2px(100));
        ...
        mWindPaint.setStrokeWidth(dp2px(3));
        ...
        mAnimation = new Animation() {
            ...
            public void applyTransformation(float interpolatedTime, @NonNull Transformation t) {
                ...
                mLoadingAnimationTime = LOADING_ANIMATION_COEFFICIENT * (interpolatedTime
                    / SLOW_DOWN_ANIMATION_COEFFICIENT);
                thisView.invalidate();
            }
        };
...
//        cloudCenter.printOriginal("cloudCenter");
        ...
        mAirplane.setBounds(0, 0, dp2px(65), dp2px(20));
        mCloudCenter.setBounds(0, 0, dp2px(260), dp2px(45));
        //</editor-fold>
...
//    }
        ...
        if (isRefreshing) {
            ...
            while (mWinds.size() < WIND_SET_AMOUNT) {
                ...
                if (mWinds.size() > 1) {
                    y = 0;
                    while (y == 0) {
                        float tmp = (float) (mHeaderHeight / (Math.random()
                        ...
                        for (Map.Entry<Float, Float> wind : mWinds.entrySet()) {
                            ...
                            if (Math.abs(wind.getKey() - tmp)
                                > mHeaderHeight / RANDOM_Y_COEFFICIENT) {
                                y = tmp;
                            } else {
                                y = 0;
                                break;
    ...
    private void drawWind(Canvas canvas, float y, float xOffset, int width) {
        ...
        */
        float cof = (width + xOffset) / (1f * LOADING_ANIMATION_COEFFICIENT
            / SLOW_DOWN_ANIMATION_COEFFICIENT);
        ...
        if (mLastAnimationTime - mLoadingAnimationTime > 0) {
            ...
            time = (1f * LOADING_ANIMATION_COEFFICIENT / SLOW_DOWN_ANIMATION_COEFFICIENT)
    ...
    private void drawSideClouds(Canvas canvas, int width, int height) {
        ...
        if (scalePercentDelta > 0) {
            ...
            scale = SIDE_CLOUDS_INITIAL_SCALE
                + (SIDE_CLOUDS_FINAL_SCALE - SIDE_CLOUDS_INITIAL_SCALE) * scalePercent;
        } else {
            scale = SIDE_CLOUDS_INITIAL_SCALE;
        }
...
//        }
        ...
        if (isRefreshing) {
            if (checkCurrentAnimationPart(AnimationPart.FIRST)) {
                offsetLeftX -=
                    2 * getAnimationPartValue(AnimationPart.FIRST) / Y_SIDE_CLOUDS_SLOW_DOWN_COF;
                offsetRightX +=
                    getAnimationPartValue(AnimationPart.FIRST) / X_SIDE_CLOUDS_SLOW_DOWN_COF;
            } else if (checkCurrentAnimationPart(AnimationPart.SECOND)) {
                offsetLeftX -=
                    2 * getAnimationPartValue(AnimationPart.SECOND) / Y_SIDE_CLOUDS_SLOW_DOWN_COF;
                offsetRightX +=
                    getAnimationPartValue(AnimationPart.SECOND) / X_SIDE_CLOUDS_SLOW_DOWN_COF;
            } else if (checkCurrentAnimationPart(AnimationPart.THIRD)) {
                offsetLeftX -=
                    getAnimationPartValue(AnimationPart.THIRD) / Y_SIDE_CLOUDS_SLOW_DOWN_COF;
                offsetRightX +=
                    2 * getAnimationPartValue(AnimationPart.THIRD) / X_SIDE_CLOUDS_SLOW_DOWN_COF;
            } else if (checkCurrentAnimationPart(AnimationPart.FOURTH)) {
                offsetLeftX -=
                    getAnimationPartValue(AnimationPart.FOURTH) / X_SIDE_CLOUDS_SLOW_DOWN_COF;
                offsetRightX +=
                    2 * getAnimationPartValue(AnimationPart.FOURTH) / Y_SIDE_CLOUDS_SLOW_DOWN_COF;
            }
        }
        ...
        matrix.postScale(scale, scale, mCloudLeft.getBounds().width() * 3 / 4f,
            mCloudLeft.getBounds().height());
    ...
    private void drawCenterClouds(Canvas canvas, int width, int height) {
        ...
        if (scalePercentDelta > 0) {
            ...
            scale = CENTER_CLOUDS_INITIAL_SCALE
                + (CENTER_CLOUDS_FINAL_SCALE - CENTER_CLOUDS_INITIAL_SCALE) * scalePercent;
        } else {
            scale = CENTER_CLOUDS_INITIAL_SCALE;
        }
        ...
        float offsetY = dragYOffset
            - (parallax ? mCloudCenter.getBounds().height() / 2f + parallaxPercent
            : mCloudCenter.getBounds().height() / 2f);
    ...
    private float getAnimationPartValue(AnimationPart part) {
        switch (part) {
            ...
            case SECOND: {
                return getAnimationTimePart(AnimationPart.FOURTH) - (mLoadingAnimationTime
                    - getAnimationTimePart(AnimationPart.FOURTH));
            }
            case THIRD: {
                return mLoadingAnimationTime - getAnimationTimePart(AnimationPart.SECOND);
            }
            case FOURTH: {
                return getAnimationTimePart(AnimationPart.THIRD) - (mLoadingAnimationTime
                    - getAnimationTimePart(AnimationPart.FOURTH));
            }
            default:
                return 0;

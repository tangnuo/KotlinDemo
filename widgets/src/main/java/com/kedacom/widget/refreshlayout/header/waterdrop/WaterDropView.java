package com.kedacom.widget.refreshlayout.header.waterdrop;
...
/**
 * 下拉头中间的 “水滴” from https://github.com/THEONE10211024/WaterDropListView
 */
...
public class WaterDropView extends View {
    ...
    public WaterDropView(Context context) {
        ...
        mPaint.setStrokeWidth(STROKE_WIDTH = SystemUtil.dp2px(1f));
        ...
        mPaint.setColor(Color.GRAY);
        mMaxCircleRadius = SystemUtil.dp2px(20);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        super.setMeasuredDimension(width + thisView.getPaddingLeft() + thisView.getPaddingRight(),
            View.resolveSize(height + thisView.getPaddingTop() + thisView.getPaddingBottom(),
                heightMeasureSpec));
    }
...
//        canvas.drawCircle(bottomCircle.x, bottomCircle.y, bottomCircle.radius, mPaint);
    ...
    private void makeBezierPath() {
        ...
        if (bottomCircle.y > topCircle.y + SystemUtil.dp2px(1)) {
            mPath
                .addCircle(bottomCircle.x, bottomCircle.y, bottomCircle.radius, Path.Direction.CCW);
            //获取两圆的两个切线形成的四个切点
...
//            throw new IllegalStateException("bottomCircle's radius must be less than the topCircle's");
    ...
    public ValueAnimator createAnimator() {
        ValueAnimator valueAnimator = ValueAnimator.ofFloat(1, 0.001f)
            .setDuration(BACK_ANIM_DURATION);
...
//        updateCompleteState(Math.max(0, 1f * (offset - space) / (maxHeight - space)));
    ...
    public void updateCompleteState(int height) {
        ...
        if (height < space) {
            ...
            float y = (float) (limit * (1 - Math.pow(100, -x / SystemUtil.dp2px(200))));

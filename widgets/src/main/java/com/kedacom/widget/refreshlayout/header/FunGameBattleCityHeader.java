...
import com.kedacom.widget.refreshlayout.header.fungame.FunGameView;
...
public class FunGameBattleCityHeader extends FunGameView {
    ...
    /**
     * 当前前一辆敌方坦克和后一辆已经存在的间距值 用于确定是否要派出新的一辆敌方坦克
     */
    protected int offsetETankX;

    /**
     * 当前前一颗子弹和后一颗子弹的间距值 用于确定是否要发射新的一颗子弹
     */
    ...
    protected void drawGame(Canvas canvas, int width, int height) {
        ...
        if (thisView.isInEditMode()) {
            ...
            drawTank(canvas, new RectF(controllerSize * 3, controllerSize * 2, controllerSize * 4,
                controllerSize * 3));
        }
    }
    ...
    protected void drawSelfTank(Canvas canvas, int width) {
        ...
        boolean isAboveCrash = checkTankCrash(getTrackIndex((int) controllerPosition),
            ...
            controllerPosition);
        boolean isBelowCrash = checkTankCrash(
            getTrackIndex((int) (controllerPosition + controllerSize)),
            width - controllerSize,
    ...
    protected void drawTank(Canvas canvas, RectF rectF) {
        ...
        canvas.drawRect(rectF.right, barrelTop, rectF.right + barrelSize, barrelTop + barrelSize,
            mPaint);

...
public class BezierRadarHeader extends InternalAbstract implements RefreshHeader {
    ...
    public BezierRadarHeader(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mDotRadius = SystemUtil.dp2px(7);
        mRadarRadius = SystemUtil.dp2px(20);
        mRadarCircle = SystemUtil.dp2px(7);
        mPaint.setStrokeWidth(SystemUtil.dp2px(3));

        thisView.setMinimumHeight(SystemUtil.dp2px(100));
        ...
        mEnableHorizontalDrag = ta.getBoolean(R.styleable.BezierRadarHeader_srlEnableHorizontalDrag,
...
//        }
    ...
    protected void drawWave(Canvas canvas, int width) {
        ...
        mPath.quadTo(mWaveOffsetX >= 0 ? (mWaveOffsetX) : width / 2, mWaveTop + mWaveHeight, width,
            mWaveTop);
    ...
    protected void drawDot(Canvas canvas, int width, int height) {
        if (mDotAlpha > 0) {
            ...
            final int num = 7;
            float x = SystemUtil.px2dp(height);
            float wide = (width / num) * mDotFraction - ((mDotFraction > 1) ? (
                (mDotFraction - 1) * (width / num) / mDotFraction)
                : 0);//y1 = t*(w/n)-(t>1)*((t-1)*(w/n)/t)
            float high =
                height - ((mDotFraction > 1) ? ((mDotFraction - 1) * height / 2 / mDotFraction)
                    : 0);//y2 = x - (t>1)*((t-1)*x/t);
            for (int i = 0; i < num; i++) {
                float index = 1f + i - (1f + num)
                    / 2;//y3 = (x + 1) - (n + 1)/2; 居中 index 变量：0 1 2 3 4 结果： -2 -1 0 1 2
                float alpha = 255 * (1 - (2 * (Math.abs(index)
                    / num)));//y4 = m * ( 1 - 2 * abs(y3) / n); 横向 alpha 差
                mPaint.setAlpha((int) (mDotAlpha * alpha * (1d - 1d / Math
                    .pow((x / 800d + 1d), 15))));//y5 = y4 * (1-1/((x/800+1)^15));竖直 alpha 差
                float radius =
                    mDotRadius * (1 - 1 / ((x / 10 + 1)));//y6 = mDotRadius*(1-1/(x/10+1));半径
    ...
    protected void drawRadar(Canvas canvas, int width, int height) {
        ...
        if (/*mRadarAnimator != null*/mAnimatorSet != null || thisView.isInEditMode()) {
            ...
            mPaint.setStyle(Paint.Style.FILL);
            mRadarRect.set(width / 2 - radius, height / 2 - radius, width / 2 + radius,
                height / 2 + radius);
...
//    }
    ...
    @Override
    public void onReleased(@NonNull final RefreshLayout refreshLayout, int height,
        int maxDragHeight) {
        mWaveTop = height;
        mWavePulling = false;

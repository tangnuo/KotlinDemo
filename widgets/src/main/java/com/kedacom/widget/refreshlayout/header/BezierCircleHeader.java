...
/**
 * CircleRefresh from https://github.com/tuesda/CircleRefreshLayout
 */
public class BezierCircleHeader extends InternalAbstract implements RefreshHeader {
    ...
    public BezierCircleHeader(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mOuterPaint.setStrokeWidth(SystemUtil.dp2px(2f));
        mPath = new Path();
    }
    ...
    private void drawBollTail(Canvas canvas, int viewWidth, float fraction) {
        if (mShowBollTail) {
            ...
            final float startX = viewWidth / 2f + (float) Math
                .sqrt(mBollRadius * mBollRadius * (1 - fraction * fraction / 4));
    ...
    private void drawOuter(Canvas canvas, int viewWidth) {
        if (mShowOuter) {
            ...
            canvas.drawArc(
                new RectF(viewWidth / 2f - outerR, mBollY - outerR, viewWidth / 2f + outerR,
                    mBollY + outerR),
    ...
    private void drawFinish(Canvas canvas, int viewWidth) {
        if (mFinishRatio > 0) {
            ...
            if (mFinishRatio < 0.3) {
                canvas.drawCircle(viewWidth / 2f, mBollY, mBollRadius, mFrontPaint);
                int outerR = (int) (mBollRadius + mOuterPaint.getStrokeWidth() * 2 * (1
                    + mFinishRatio / 0.3f));
                int afterColor = ColorUtils
                    .setAlphaComponent(beforeColor, (int) (0xff * (1 - mFinishRatio / 0.3f)));
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                if (status == 2) {
                    if (mBollY > mHeadHeight / 2) {
                        ...
                        float bally =
                            animation.getAnimatedFraction() * (mHeadHeight / 2 - springBollY)
                                + springBollY;
                        if (mBollY > bally) {
                            mBollY = bally;
                        }

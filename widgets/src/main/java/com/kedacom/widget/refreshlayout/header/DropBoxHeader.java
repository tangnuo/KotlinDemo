...
/**
 * DropBoxRefresh design https://dribbble.com/shots/3470499-DropBox-Refresh
 */
...
public class DropBoxHeader extends InternalAbstract implements RefreshHeader {
    ...
    public DropBoxHeader(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        thisView.setMinimumHeight(dp2px(150));
...
//            drawable3.printOriginal("drawable3");
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mDropOutPercent > 0) {
            ...
            bounds1.offsetTo(width / 2 - bounds1.width() / 2,
                (int) ((body.boxCenterY - bounds1.height() / 2 + bounds1.height()) * percent1)
                    - bounds1.height());
            mDrawable1.draw(canvas);

            final float percent2 = Math.min(Math.max(mDropOutPercent - 1, 0), 1);
            Rect bounds2 = mDrawable2.getBounds();
            bounds2.offsetTo(width / 2 - bounds2.width() / 2,
                (int) ((body.boxCenterY - bounds2.height() / 2 + bounds2.height()) * percent2)
                    - bounds2.height());
            mDrawable2.draw(canvas);

            final float percent3 = Math.min(Math.max(mDropOutPercent - 2, 0), 1);
            Rect bounds3 = mDrawable3.getBounds();
            bounds3.offsetTo(width / 2 - bounds3.width() / 2,
                (int) ((body.boxCenterY - bounds3.height() / 2 + bounds3.height()) * percent3)
                    - bounds3.height());
            mDrawable3.draw(canvas);

            if (mDropOutOverFlow) {
                bounds1.offsetTo(width / 2 - bounds1.width() / 2,
                    ((body.boxCenterY - bounds1.height() / 2)));
                mDrawable1.draw(canvas);

                bounds2.offsetTo(width / 2 - bounds2.width() / 2,
                    ((body.boxCenterY - bounds2.height() / 2)));
                mDrawable2.draw(canvas);

                bounds3.offsetTo(width / 2 - bounds3.width() / 2,
                    ((body.boxCenterY - bounds3.height() / 2)));
                mDrawable3.draw(canvas);
            }
        }
    ...
    private Path generateBoxCoverPath(BoxBody body) {
        ...
        mPath.lineTo(body.boxCenterX - offsetLeftBottomX,
            (body.boxBottom + body.boxTop) / 2f + offsetLeftBottomY);
        ...
        mPath.moveTo(body.boxRight, body.boxCenterTop);
        mPath.lineTo(body.boxCenterX, (body.boxBottom + body.boxTop) / 2f);
        mPath.lineTo(body.boxCenterX + offsetRightBottomX,
            (body.boxBottom + body.boxTop) / 2f + offsetRightBottomY);
    ...
    private Path generateBoxBodyPath(BoxBody body) {
        ...
        mPath.quadTo(body.boxRight + body.boxSideLength / 2f * mReboundPercent, body.boxCenterY,
            body.boxRight, body.boxCenterTop);
        mPath.lineTo(body.boxCenterX, body.boxTop);
        mPath.lineTo(body.boxLeft, body.boxCenterTop);
        mPath.quadTo(body.boxLeft - body.boxSideLength / 2f * mReboundPercent, body.boxCenterY,

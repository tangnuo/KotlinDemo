...
package com.kedacom.widget.refreshlayout.header.internal.pathview;
...
class PathParser {
     ...
     * @return the new array //     * @throws ArrayIndexOutOfBoundsException if {@code start < 0 ||
     * start > original.length} //     * @throws IllegalArgumentException       if {@code start >
     * end}
     */
...
//        }
    ...
    public static List<Path> transformScale(float ratioWidth, float ratioHeight,
        List<Path> originPaths, List<String> orginSvgs) {
    ...
    private static void transformScaleNodes(float ratioWidth, float ratioHeight,
        PathDataNode[] node) {
    ...
    private static void transformScaleCommand(float ratioWidth, float ratioHeight, char cmd,
        float[] val) {
        int inc = 2;
        switch (cmd) {
            case 'z':
...
//            if (nodesFrom[i].value != nodesTo[i].value

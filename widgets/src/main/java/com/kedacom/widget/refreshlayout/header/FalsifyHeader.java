...
/**
 * 虚假的 Header 用于 正真的 Header 在 RefreshLayout 外部时， 使用本虚假的 FalsifyHeader 填充在 RefreshLayout 内部 具体使用方法 参考
 * 纸飞机（FlyRefreshHeader）
 */
...
public class FalsifyHeader extends InternalAbstract implements RefreshHeader {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (thisView.isInEditMode()) {//这段代码在运行时不会执行，只会在Studio编辑预览时运行，不用在意性能问题
            int d = SystemUtil.dp2px(5);

            Paint paint = new Paint();
            paint.setStyle(Paint.Style.STROKE);
            paint.setColor(0xcccccccc);
            paint.setStrokeWidth(SystemUtil.dp2px(1));
            ...
            textView.setText(thisView.getResources()
                .getString(R.string.srl_component_falsify, getClass().getSimpleName(),
                    SystemUtil.px2dp(thisView.getHeight())));
            textView.setTextColor(0xcccccccc);
            textView.setGravity(Gravity.CENTER);
            //noinspection UnnecessaryLocalVariable
            View view = textView;
            view.measure(makeMeasureSpec(thisView.getWidth(), EXACTLY),
                makeMeasureSpec(thisView.getHeight(), EXACTLY));
